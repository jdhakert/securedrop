...
The test suite already comes with a test runner (``test_alembic.py``) hat run a series of checks to
ensure migration's upgrade and downgrade commands are idempotent and don't break the database. The
test runner uses dynamic module import to iterate through all the migrations. You will need to
...
Your module **MUST** contain the classes with the following classes with the following attributes.
